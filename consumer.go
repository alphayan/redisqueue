...
func (c *Consumer) Run() {
	...
	for stream, consumer := range c.consumers {
		...
		err := c.redis.XGroupCreateMkStream(rctx, stream, c.options.GroupName, consumer.id).Err()
	...
	for {
		...
		case <-ticker.C:
			for stream := range c.consumers {
				...
				for {
					res, err := c.redis.XPendingExt(rctx, &redis.XPendingExtArgs{
						Stream: stream,
					...
					for _, r := range res {
						if r.Idle >= c.options.VisibilityTimeout {
							claimres, err := c.redis.XClaim(rctx, &redis.XClaimArgs{
								Stream:   stream,
								Group:    c.options.GroupName,
							...
							if err == redis.Nil {
								err = c.redis.XAck(rctx, stream, c.options.GroupName, r.ID).Err()
								if err != nil {
...
func (c *Consumer) poll() {
	for {
		...
		case <-c.stopPoll:
			...
			return
		default:
			res, err := c.redis.XReadGroup(rctx, &redis.XReadGroupArgs{
				Group:    c.options.GroupName,
				Consumer: c.options.Name,
				Streams:  c.streams,
				Count:    int64(c.options.BufferSize - len(c.queue)),
...
func (c *Consumer) work() {
	...
	for {
		...
		case msg := <-c.queue:
			...
			err = c.redis.XAck(rctx, msg.Stream, c.options.GroupName, msg.ID).Err()
			if err != nil {
